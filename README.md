...
예산은 Supervisor가 파견할 때 지정합니다(§3.1). 값이 한 곳에 모여 있어 어디서
정해지는지 추적됩니다.

| 구간 | 상한 |
|---|---|
| 그래프 전체 | `recursion_limit=60` (허브 홉 포함) |
| Researcher ReAct | 4턴 |
| 근거 공백 재수집 | 2턴 × 최대 1회 |
| Analyst ReAct | 3턴 |
| Reviewer 피드백 후 재분석 | 2턴 |
| Reviewer → Analyst 재검토 | 최대 1회 |
...
| 계층 | 재는 것 | LLM | 실행 |
|---|---|---|---|
| 단위 테스트 (101개) | 라우팅 정책, 근거 원장, 근거 대조, 컨텍스트 압축, 후기 검색, 그래프 제어 흐름, 데이터 계층 | 불필요 (대역 주입) | CI, ~20초 |
...
- **Verifier 결과를 자동 조치로 연결하지 않은 것은 한계가 아니라 의도입니다.** 대조율은 파이프라인이 최적화하지 않는 독립 관측치로 둡니다(§3.7) — 루프를 걸면 파이프라인이 이 수치를 향해 최적화하고, 그 순간 "근거에 충실한가"를 재는 지표가 아니라 "루프를 피하는 법"을 학습한 결과가 됩니다. 게다가 대조율이 낮은 주된 원인은 파생 수치라 재수집으로 고쳐지지 않고, 근거가 진짜 없는 경우는 §3.1의 공백 규칙이 수집 직후에 훨씬 싸게 잡습니다. 연결하려면 먼저 파생 수치와 환각을 분리해야 하며, 분리된 뒤에도 처방은 재수집이 아니라 Synthesis 재작성입니다 — **재수집은 "없는 데이터"의 처방이지 "잘못 쓴 문장"의 처방이 아닙니다.**
...
tests/                    라우팅 / 허브 재계획 / 원장 / 대조 / 압축 / 후기검색 / 그래프흐름 / 계측 / 데이터계층
